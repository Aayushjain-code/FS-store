							...
							{loggedIn ? <>
								{
									cart.length !== 0 && cart.some((cartItem) => cartItem._id === item._id) ?
										<button class="button btn-success" onClick={redirectToCart}>Go to Cart</button>
										:
										<button class="button btn-success" onClick={() => item.availableOrNot && addToCart(item, 'Added to Cart')}>Buy Now</button>

								}

								{
									wishlist.length !== 0 && wishlist.some((wishlistItem) => wishlistItem._id === item._id) ?
										<button class="button btn-primary" onClick={redirectToWishlist}>Wishlist </button>
										:
										<button class="button btn-primary" onClick={() => item.availableOrNot && addToWishlist(item, 'Added to Wishlist')}>Add Wishlist </button>

								}

								{/* <button class="button btn-primary" onClick={() => addToWishlist(item, 'Added to Wishlist')}>Wishlist</button> */}
							</> : <>
								<button className={item.availableOrNot ? "button btn-success" : "button btn-success btn-disabled"} onClick={redirectToLogin} >Buy Now</button>
								<button className={item.availableOrNot ? "button btn-primary" : "button btn-primary btn-disabled"} onClick={redirectToLogin}>Wishlist</button>
